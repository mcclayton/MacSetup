...
function! s:fzf_file_state_script(state) abort
  ...
  \ '}',
  \ '',
  \ 'paced_file_reload_action() {',
  \ '  printf "%s" "reload(sleep $file_filter_delay; \"$script_file\" files-for-search || true)"',
  \ '}',
  \ '',
  \ 'header() {',
  ...
  \ 'live_filter_actions() {',
  \ '  case "$FZF_PROMPT" in',
  \ '    "Include > ") write_current_query "$include_file"; printf "%s\n" "$(paced_file_reload_action)+transform-header(\"$script_file\" header)" ;;',
...
function! s:fzf_rg_state_script(state) abort
  ...
  \ '  printf "%s" "reload(sleep $rg_filter_delay; \"$script_file\" $action || true)"',
  \ '}',
  \ '',
  \ 'header() {',
  \ '  include=$(read_file "$include_file")',
  ...
  \ 'live_filter_actions() {',
  \ '  case "$FZF_PROMPT" in',
  \ '    "Search > ") write_current_query "$search_file"; printf "%s\n" "$(paced_saved_search_reload)+transform-header(\"$script_file\" header)" ;;',
  \ '    "Include > ") write_current_query "$include_file"; printf "%s\n" "$(paced_saved_search_reload)+transform-header(\"$script_file\" header)" ;;',
  \ '    "Exclude > ") write_current_query "$exclude_file"; printf "%s\n" "$(paced_saved_search_reload)+transform-header(\"$script_file\" header)" ;;',
  \ '    *) printf "%s\n" "transform-header(\"$script_file\" header)" ;;',
  \ '  esac',
