...
function! s:fzf_file_state_script(state) abort
  ...
  \ 'script_file="$0"',
  \ 'state_dir=' . shellescape(a:state.dir),
  \ 'file_filter_delay=0.15',
  \ '',
  \ 'read_file() {',
  \ '  [ -f "$1" ] && cat "$1"',
  \ '}',
  ...
  \ '}',
  \ '',
  \ 'snapshot_current_query() {',
  \ '  snapshot=$(mktemp "$state_dir/reload.XXXXXX") || return 1',
  \ '  printf "%s" "$FZF_QUERY" > "$snapshot"',
  \ '  printf "%s" "$snapshot"',
  \ '}',
  \ '',
  \ 'split_patterns() {',
  ...
  \ '}',
  \ '',
  \ 'debounced_file_reload_action() {',
  \ '  target_file="$1"',
  \ '  snapshot=$(snapshot_current_query) || { printf "%s" "transform-header(\"$script_file\" header)"; return; }',
  \ '  printf "%s" "reload(sleep $file_filter_delay; cmp -s \"$target_file\" \"$snapshot\" && \"$script_file\" files-for-search || true; rm -f \"$snapshot\")"',
  \ '}',
  \ '',
  \ 'header() {',
  ...
  \ 'live_filter_actions() {',
  \ '  case "$FZF_PROMPT" in',
  \ '    "Include > ") write_current_query "$include_file"; printf "%s\n" "$(debounced_file_reload_action "$include_file")+transform-header(\"$script_file\" header)" ;;',
  \ '    "Exclude > ") write_current_query "$exclude_file"; printf "%s\n" "$(debounced_file_reload_action "$exclude_file")+transform-header(\"$script_file\" header)" ;;',
...
function! s:fzf_rg_state_script(state) abort
  ...
  \ '',
  \ 'saved_search_reload() {',
  \ '  query=$(read_file "$search_file")',
  \ '  if [ -n "$query" ]; then',
  \ '    printf "%s" "reload(\"$script_file\" $(saved_search_action) || true)"',
  \ '  else',
  \ '    printf "%s" "reload(printf \"\")"',
  \ '  fi',
  \ '}',
  \ '',
  \ 'debounced_saved_search_reload() {',
  \ '  target_file="$1"',
  \ '  query=$(read_file "$search_file")',
  \ '  if [ -z "$query" ]; then',
  \ '    printf "%s" "reload(printf \"\")"',
  ...
  \ '  printf "%s" "reload(sleep $rg_filter_delay; cmp -s \"$target_file\" \"$snapshot\" && \"$script_file\" $action || true; rm -f \"$snapshot\")"',
  \ '}',
  \ '',
  \ 'header() {',
  \ '  include=$(read_file "$include_file")',
  ...
  \ '',
  \ 'search_actions() {',
  \ '  printf "%s\n" "change-prompt(Search > )+disable-search+transform-query(cat \"$search_file\")+$(saved_search_reload)+transform-header(\"$script_file\" header)"',
  \ '}',
  \ '',
  \ 'edit_include_actions() {',
  \ '  printf "%s\n" "change-prompt(Include > )+transform-query(cat \"$include_file\")+$(saved_search_reload)+transform-header(\"$script_file\" header)"',
  \ '}',
  \ '',
  \ 'edit_exclude_actions() {',
  \ '  printf "%s\n" "change-prompt(Exclude > )+transform-query(cat \"$exclude_file\")+$(saved_search_reload)+transform-header(\"$script_file\" header)"',
  \ '}',
  \ '',
  \ 'live_filter_actions() {',
  \ '  case "$FZF_PROMPT" in',
  \ '    "Search > ") write_current_query "$search_file"; printf "%s\n" "$(debounced_saved_search_reload "$search_file")+transform-header(\"$script_file\" header)" ;;',
  \ '    "Include > ") write_current_query "$include_file"; printf "%s\n" "$(debounced_saved_search_reload "$include_file")+transform-header(\"$script_file\" header)" ;;',
  \ '    "Exclude > ") write_current_query "$exclude_file"; printf "%s\n" "$(debounced_saved_search_reload "$exclude_file")+transform-header(\"$script_file\" header)" ;;',
  \ '    *) printf "%s\n" "transform-header(\"$script_file\" header)" ;;',
  \ '  esac',
  \ '}',
  \ '',
...
function! s:fzf_rg_options(query, include, exclude, state) abort
  ...
  \ '--bind', 'change:transform:' . s:fzf_rg_state_command(a:state, 'change'),
...
function! s:hotkey_lines() abort
  ...
  \ '  <C-h>        Show this full hotkey reference',
  ...
  \ '  <C-w>        End of line',
  \ '  Scope        Insert mode only; normal-mode Ctrl keys keep Vim behavior',
  \ '',
  \ 'Command Palette',
  \ '  <C-p>        Open palette',
  \ '  j/k          Move selection',
...
"Ctrl+h to show custom and plugin hotkeys in normal/visual mode
nnoremap <silent> <C-h> :Hotkeys<CR>
vnoremap <silent> <C-h> :<C-u>Hotkeys<CR>
...
"Match the shell Ctrl+WASD line-editing bindings in insert mode.
inoremap <silent> <C-a> <C-o>b
inoremap <silent> <C-d> <C-o>w
